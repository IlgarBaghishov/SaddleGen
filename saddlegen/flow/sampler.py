...
def sample_saddles(
    ...
    dimer_residual_alpha: torch.Tensor | float | None = None,
    ...
    **v7-2a:** integration starts at the (R, P) midpoint instead of at R, and
    ...
    _use_dimer_residual_check = dimer_residual_alpha is not None
    ...
    if want_endpoint_features:
        ...
        if not isinstance(backbone, TimeFiLMBackbone):
            raise RuntimeError(
                "v7-2b endpoint features need a TimeFiLMBackbone (forward_static)."
            )
    ...
    for step in range(K):
        ...
        use_dimer_residual = (
            dimer_residual_alpha is not None and eigenmode_head is not None
        )
        ...
        with torch.no_grad():
            ...
            # v7-4-redesign: output-side Dimer nudge with learned scalar α.
            if use_dimer_residual and dimer_force_all_inf is not None:
                alpha = float(dimer_residual_alpha) if not torch.is_tensor(dimer_residual_alpha) else dimer_residual_alpha.item()
                v = v + alpha * dimer_force_all_inf
